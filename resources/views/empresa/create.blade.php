      ...
      <div class="col-md-12">
        ...
        <form id="formempresa" class="form" method="POST" action="{{ ! $empresa->id ? route('empresas.store') : route('empresas.update', $empresa->id)}}" enctype="multipart/form-data">
          ...
          <div class="form-group">
            ...
            <textarea name="servicios" required id="servicios" class="form-control   {{ $errors->has('servicios') ? ' is-invalid' : '' }}" rows="5" aria-describedby="descriptionHelp">{{ old('servicios') ?: $empresa->servicios }}</textarea><small id="descriptionHelp" class="form-text text-muted mt-2">Especifica tus servicios separados por un guión (-).</small>
            @error('servicios')
            <span class="invalid-feedback" role="alert">
              <strong>{{ $message }}</strong>
            </span>
